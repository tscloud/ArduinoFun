...
void loop() {
    ...
    // -- Encoder - should we do this upfront or do other stuff 1st?
    newPosition = myEnc.read();
    if (newPosition != oldPosition) {
      //Serial.println(newPosition);
      // -- this is a big deal - we've got to "de-quadrify" the rotary encoder
      if ((newPosition % 4) == 0) {
        if (newPosition > oldPosition) {
            bigPos += 1;
        }
        ...
        if (blankoutCounter < blankoutThresh) {
          ...
          if (displayTrackedValue) {
            ...
            else if (!encDirUp && (trackedValue > valueMin)) {
              trackedValue--;
              EEPROM.write(addr, trackedValue);
              EEPROM.commit();
            }
...
void handleInterruptDisplay() {
  if((long)(micros() - last_micros) >= debouncing_time * 1000) {
    ...
    if (blankoutCounter < blankoutThresh) {
      // toggle b/w display of environment data & tracked value
      if (displayTrackedValue) {
